...
public class TableControler implements Initializable {
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		try {
			...
			ResultSet rs=con.createStatement().executeQuery("select * from pro_data");
			while (rs.next()) {
				oblist.add(new ModelTable(rs.getString("id"), rs.getString("name"), rs.getString("email"), rs.getString("prog")));
			}
